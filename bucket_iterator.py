...
class BucketIterator(object):
    ...
    def pad_data(self, batch_data):
        ...
        batch_bert_indices = []
        batch_box_indices = []
        # batch_caption_indices = []

        if True:
            bert_indices_max_len = max([(len(t['bert_indices'])+len(t['caption_indices'])) for t in batch_data])
            box_indices_max_len = max([sum([len(x_) for x_ in t["box_indices"]]) for t in batch_data])
            vit_size = max([len(t["box_vit"]) for t in batch_data])    # 这个维度应该都是768
            ...
            for item in batch_data:
                ...
                # 将每一个graph的维度 扩展到与最大bert length相同的维度
                ...
                # 当后面对图的size一致性有要求时再修改这部分    temp等同于image_graph.shape[1]-2  可以输出试一下看看
                temp_box_indices_len = sum(len(x) for x in box_indices)
                graph = numpy.pad(graph, ((0, 10), (0, 10)), 'constant')
                # 此处的作用： 把text的图扩展10个维度 方便与image graph合并
                #graph = numpy.pad(graph,((0,5),(0,5)),'constant')

                for i in range(image_graph.shape[0]-2):
                    for j in range(image_graph.shape[1]):
                        if not numpy.isnan(image_graph[i+1][j]):    # 此处的[i  应该+1][]  否则image graph会取不到倒数第二行
                ...
                bert_indices = bert_indices+caption_indices
                batch_bert_indices.append(numpy.pad(bert_indices,(0,bert_indices_max_len - len(bert_indices)),'constant'))
                # 此处修改   在jupyter中尝试看看属性是否需要分开
                # batch_caption_indices.append(numpy.pad(caption_indices,(0,bert_indices_max_len - len(caption_indices)),'constant'))
            ...
            # print(torch.tensor(batch_graph).shape, torch.tensor(batch_caption_indices).shape, torch.tensor(batch_bert_indices).shape)
            return {
                ...
                # 'caption_indices':torch.tensor(batch_caption_indices)
                }
